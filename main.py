...
def main():
    ...
    if cdn_configs:
        ...
        variants = generate_port_variants(cdn_configs[:20])
        all_cdn = cdn_configs + variants

        cdn_tester = CDNTester()
        cdn_tested = cdn_tester.test_batch(all_cdn)
        cdn_best = cdn_tester.get_best(cdn_tested, top_n=500)

        if cdn_best:
            cdn_best = fix_all_configs(cdn_best)
            cdn_best = diversify_ports(cdn_best, 0.4)[:500]
